# Эти задачи необходимо решить используя регулярные выражения!

# Задача - 1
# Запросите у пользователя имя, фамилию, email. Теперь необходимо совершить проверки, имя и фамилия должны иметь заглавные первые буквы.
# email - не должен иметь заглавных букв и должен быть в формате: текст в нижнем регистре, допускается нижнее подчеркивание и цифры, потом @, потом текст, допускаются цифры, точка, ru или org или com.
# Например:
# Пупкин василий - неверно указано имя, te$T@test.net - неверно указан email (спецсимвол, заглавная буква, .net), te_4_st@test.com - верно указан.

import re
print('# Задание-1:')
name_surname = input('Введите имя и фамилию: ')
email = input('Введите e-mail: ')

pattern = '([a-zA-Z_0-9]+@[a-z]+\.ru|org|com)'
#email = 'alex@mail.ru'
#name_surname = '  Dadsha    FMMdrd  '
ns_lst = re.sub('\s+', ' ', name_surname.strip()).split(' ')
try:
    name = ns_lst[0]
except:
    name = ''
try:
    surname = ns_lst[1]
except:
    surname = ''
pattern = '^([A-Z]{1}[a-z]+)$|^([А-Я]{1}[а-я]+)$'
print('Имя указано верно,' if re.match(pattern, name)
      else '​Имя указано неверно,',
      'фамилия указано верно' if re.match(pattern, surname)
      else '​фамилия указана неверно')

email_pattern = '([a-z_0-9]+@[a-z]+\.ru|org|com)'
#email_pattern = '^([\w]+@[\w]+\.ru|org|com)$'
print('email указан верно' if re.match(email_pattern, email)
      else '​email указан неверно')
print('------------')

# Задача - 2:
# Вам дан текст:

some_str = '''
Мороз и солнце; день чудесный!
Еще ты дремлешь, друг прелестный —
Пора, красавица, проснись:
Открой сомкнуты негой взоры
Навстречу северной Авроры,
Звездою севера явись!
Вечор, ты помнишь, вьюга злилась,
На мутном небе мгла носилась;
Луна, как бледное пятно,
Сквозь тучи мрачные желтела,
И ты печальная сидела —
А нынче... погляди в окно:
Под голубыми небесами
Великолепными коврами,
Блестя на солнце, снег лежит;
Прозрачный лес один чернеет,
И ель сквозь иней зеленеет,
И речка подо льдом блестит..
Вся комната янтарным блеском
Озарена. Веселым треском
Трещит затопленная печь.
Приятно думать у лежанки.
Но знаешь: не велеть ли в санки
Кобылку бурую запречь?
Скользя по утреннему снегу,
Друг милый, предадимся бегу
Нетерпеливого коня
И навестим поля пустые,
Леса, недавно столь густые,
И берег, милый для меня..'''

# Необходимо с помощью регулярных выражений определить есть ли в тексте подряд
# более одной точки, при любом исходе сообщите результат пользователю!

print('# Задание-2:')
dot_pattern = '([\.][\.]+)'
print('В тексте встречается более одной точки подряд'
      if re.search(dot_pattern, some_str)
      else 'В тексте не встречается более одной точки подряд')
print('------------')